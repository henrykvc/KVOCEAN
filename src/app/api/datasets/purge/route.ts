...
export async function DELETE(request: Request) {
  ...
  if (!body?.id || !body?.companyName || !body?.quarterKey) {
    return NextResponse.json({ error: "완전삭제할 대상 정보가 없습니다." }, { status: 400 });
  }

  try {
    const { error } = await supabase.from("datasets").delete().eq("id", body.id);
    if (error) {
      throw error;
    }

    await supabase.from("change_logs").insert({
      action: "dataset_purged",
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "데이터 완전삭제에 실패했습니다." }, { status: 500 });
  }
}
