...
function parseAccountDictionaryEntries(value: unknown): AccountDictionaryEntry[] {
  ...
  return value.flatMap((entryValue) => {
    ...
    const item = entryValue as Partial<AccountDictionaryEntry>;
...
function parseAccountDictionaryStore(raw: string | null): AccountDictionaryEntry[] {
  if (!raw) {
    return [];
  }
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parseAccountDictionaryEntries(parsed);
    }
    if (parsed && typeof parsed === "object") {
      return parseAccountDictionaryEntries((parsed as { entries?: unknown }).entries ?? []);
    }
    return [];
  } catch {
    return [];
...
function extractAccountDictionaryEntries(savedDatasets: SavedQuarterSnapshot[]) {
  const entries = new Map<string, AccountDictionaryEntry>();
...
function mergeStoredAccountDictionaryEntries(storedEntries: AccountDictionaryEntry[], observedEntries: AccountDictionaryEntry[]) {
  const merged = new Map<string, AccountDictionaryEntry>();
...
function buildRequestedFormulaRows() {
  return [
    ...
    { 항목: "총자산회전율", 수식: "매출액 / ((3개 분기 전 자산 + 기준 분기 자산) / 2)" },
    { 항목: "매출채권회전율", 수식: "매출액 / ((3개 분기 전 매출채권 + 기준 분기 매출채권) / 2)" },
    { 항목: "매출채권회전기간", 수식: "365일 / 매출채권회전율" },
    { 항목: "재고자산회전율", 수식: "매출원가 / ((3개 분기 전 재고자산 + 기준 분기 재고자산) / 2)" },
...
export function ValidatorApp() {
  ...
  const [accountDictionaryEntriesState, setAccountDictionaryEntriesState] = useState<AccountDictionaryEntry[]>([]);
  ...
  useEffect(() => {
    ...
    setAccountDictionaryEntriesState(accountDictionary);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(STORAGE_KEYS.accountDictionary, JSON.stringify(accountDictionaryEntriesState));
  }, [mounted, accountDictionaryEntriesState]);
  ...
  const accountDictionaryEntries = useMemo(() => accountDictionaryEntriesState, [accountDictionaryEntriesState]);
            ...
            <div className="notice input-helper">
                    ...
                    : activeTab === "formulas"
                      ...
                      : activeTab === "account-db"
                        ? `검증 저장 데이터에서 누적된 원본 계정 ${accountDictionaryEntries.length}건을 관리합니다. 3줄 데이터의 상위 분류 아래 원본 계정명이 중복 없이 계속 쌓입니다.`
              ...
              <section className="overview-card report-hero-card">
                <div className="section-title">
                  <div>
                    ...
                    <p className="result-meta">OCR검증에서 저장한 데이터만 누적합니다. 3줄 데이터의 상위 분류를 기준으로 원본 계정명을 중복 없이 계속 쌓아두는 사전입니다.</p>
                  </div>
                  <div className="inline-actions">
              ...
              {!!accountDictionaryEntries.length && (
                <section className="config-card">
                  <div className="section-title">
                    <div>
                      <h3>섹션별 누적 원본 계정</h3>
                      <p className="muted">유동자산이 있으면 그 아래 원본 계정이 계속 쌓이고, 비유동자산이 있으면 그 아래 원본 계정이 계속 쌓이는 구조입니다.</p>
                    </div>
                  </div>
                  <div className="data-list grouped-data-list">
